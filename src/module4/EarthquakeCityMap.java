...
public class EarthquakeCityMap extends PApplet {
	...
	private void addKey() {	
		...
		text("Land Marker", x + 20, y + 15);
		text("Ocean Marker", x + 20, y + 35);
		
		textSize(12);
		text("Size -- Magnitude", x + 5, y + 55);
		...
		for(Marker cm : countryMarkers) {
			...
			for(Marker m : quakeMarkers) {
				if(m instanceof LandQuakeMarker) {
					String landCountry = (String)m.getProperty("country");
					if(name.equalsIgnoreCase(landCountry)) {
						countryCount++;
					}
				}
			}
